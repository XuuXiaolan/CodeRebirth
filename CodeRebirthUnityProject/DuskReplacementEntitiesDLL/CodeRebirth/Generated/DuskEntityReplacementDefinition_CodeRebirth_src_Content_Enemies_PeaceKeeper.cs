...
namespace Dusk
{
    ...
    public class DuskEntityReplacementDefinition_CodeRebirth_src_Content_Enemies_PeaceKeeper : DuskEnemyReplacementDefinition<CodeRebirth.src.Content.Enemies.PeaceKeeper>
    {
        ...
        protected override void Apply(CodeRebirth.src.Content.Enemies.PeaceKeeper PeaceKeeper)
        {
            if (this._idleAudioClips.audioClips.Length > 0)
            {
                PeaceKeeper._idleAudioClips = this._idleAudioClips;
            }
            if (PeaceKeeper._treadsSource.clip != null && this.potential_treadsSource != null)
            {
                PeaceKeeper._treadsSource.clip = this.potential_treadsSource;
            }
            if (PeaceKeeper._aggroSFX.clip != null && this.potential_aggroSFX != null)
            {
                PeaceKeeper._aggroSFX.clip = this.potential_aggroSFX;
            }
            if (PeaceKeeper.creatureVoice.clip != null && this.potentialCreatureVoice != null)
            {
                PeaceKeeper.creatureVoice.clip = this.potentialCreatureVoice;
            }
            if (PeaceKeeper.creatureSFX.clip != null && this.potentialCreatureSFX != null)
            {
                PeaceKeeper.creatureSFX.clip = this.potentialCreatureSFX;
            }
            if (this._revUpSound != null)
            {
                PeaceKeeper._revUpSound = this._revUpSound;
            }
            if (this._revDownSound != null)
            {
                PeaceKeeper._revDownSound = this._revDownSound;
            }
            if (this._bitchSlapSound != null)
            {
                PeaceKeeper._bitchSlapSound = this._bitchSlapSound;
            }
            if (this._bitchSlapStartSound != null)
            {
                PeaceKeeper._bitchSlapStartSound = this._bitchSlapStartSound;
            }
            if (this.spawnSound != null)
            {
                PeaceKeeper.spawnSound = this.spawnSound;
            }
            if (this.dieSFX != null)
            {
                PeaceKeeper.dieSFX = this.dieSFX;
            }

            Animator? animator = PeaceKeeper.GetComponentInChildren<Animator>(true);
            if (animator != null)
            {
