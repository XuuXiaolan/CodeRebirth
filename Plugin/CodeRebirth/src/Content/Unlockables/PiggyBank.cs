...
public class PiggyBank : NetworkBehaviour, IHittable
{
    ...
    public void SpawnAllCoinsAnimEvent()
    {
        if (IsServer)
        {
            ...
            {
                var coin = GameObject.Instantiate(MapObjectHandler.Instance.Merchant.MoneyPrefab, this.transform.position, this.transform.rotation, this.transform); // todo: check this parenting stuff, especially when breaking open piggy banks.
