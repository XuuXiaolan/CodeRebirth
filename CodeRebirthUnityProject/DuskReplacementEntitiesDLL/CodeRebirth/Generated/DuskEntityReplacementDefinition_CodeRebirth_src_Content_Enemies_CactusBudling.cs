...
namespace Dusk
{
    ...
    public class DuskEntityReplacementDefinition_CodeRebirth_src_Content_Enemies_CactusBudling : DuskEnemyReplacementDefinition<CodeRebirth.src.Content.Enemies.CactusBudling>
    {
        ...
        protected override void Apply(CodeRebirth.src.Content.Enemies.CactusBudling CactusBudling)
        {
            if (this._idleAudioClips.audioClips.Length > 0)
            {
                CactusBudling._idleAudioClips = this._idleAudioClips;
            }
            if (CactusBudling._rollingSource.clip != null && this.potential_rollingSource != null)
            {
                CactusBudling._rollingSource.clip = this.potential_rollingSource;
            }
            if (CactusBudling.creatureVoice.clip != null && this.potentialCreatureVoice != null)
            {
                CactusBudling.creatureVoice.clip = this.potentialCreatureVoice;
            }
            if (CactusBudling.creatureSFX.clip != null && this.potentialCreatureSFX != null)
            {
                CactusBudling.creatureSFX.clip = this.potentialCreatureSFX;
            }
            if (this.spawnSound != null)
            {
                CactusBudling.spawnSound = this.spawnSound;
            }
            if (this.dieSFX != null)
            {
                CactusBudling.dieSFX = this.dieSFX;
            }

            Animator? animator = CactusBudling.GetComponentInChildren<Animator>(true);
            if (animator != null)
            {
