...
public static class ItemConfigManager
{
        ...
        int cost)
    {
        return new ItemConfig
        {
            ...
            Value = CRConfigManager.CreateEntry(configFile, keyName, $"{itemName}", "Value", "-1,-1", $"how much {itemName} is worth when spawning, formatted as min,max where -1,-1 is the default."),
            ...
            Cost = createIsScrapItemConfig ? CRConfigManager.CreateEntry(configFile, keyName, $"{itemName}", "Cost", cost, $"Cost for {itemName} in the shop.") : null
        };
    }
        ...
        int cost)
    {
        ...
        CRConfigManager.CRConfigs[$"{keyName} | {itemName}"] = config; // i.e. `DuckSong | Grape`
    }

    public static ItemConfig GetItemConfig(string keyName, string itemName)
    {
        return (ItemConfig)CRConfigManager.CRConfigs[$"{keyName} | {itemName}"];
    }
}
