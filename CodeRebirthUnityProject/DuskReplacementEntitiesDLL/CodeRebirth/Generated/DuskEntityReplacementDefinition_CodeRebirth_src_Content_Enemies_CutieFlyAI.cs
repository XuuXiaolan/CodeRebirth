...
namespace Dusk
{
    ...
    public class DuskEntityReplacementDefinition_CodeRebirth_src_Content_Enemies_CutieFlyAI : DuskEnemyReplacementDefinition<CodeRebirth.src.Content.Enemies.CutieFlyAI>
    {
        ...
        protected override void Apply(CodeRebirth.src.Content.Enemies.CutieFlyAI CutieFlyAI)
        {
            if (this._idleAudioClips.audioClips.Length > 0)
            {
                CutieFlyAI._idleAudioClips = this._idleAudioClips;
            }
            if (CutieFlyAI.creatureVoice.clip != null && this.potentialCreatureVoice != null)
            {
                CutieFlyAI.creatureVoice.clip = this.potentialCreatureVoice;
            }
            if (CutieFlyAI.creatureSFX.clip != null && this.potentialCreatureSFX != null)
            {
                CutieFlyAI.creatureSFX.clip = this.potentialCreatureSFX;
            }
            if (this.spawnSound != null)
            {
                CutieFlyAI.spawnSound = this.spawnSound;
            }
            if (this.dieSFX != null)
            {
                CutieFlyAI.dieSFX = this.dieSFX;
            }

            Animator? animator = CutieFlyAI.GetComponentInChildren<Animator>(true);
            if (animator != null)
            {
